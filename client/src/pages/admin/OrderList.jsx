...
const OrderList = () => {
  ...
  const fetchOrders = async () => {
    try {
      const { data } = await axios.get(URI + "/api/admin/orders", {
  ...
  return (
    <div className="p-4 md:p-10 space-y-6 min-h-screen bg-white">
      <h2 className="text-2xl font-bold text-gray-700 mb-6">🛍️ Orders List</h2>
      ...
      {orders.length === 0 && (
        <p className="text-center text-gray-600 text-base mt-4">
          There are currently no orders in the system. Once customers start
          placing orders, they will appear here.
        </p>
      )}
    </div>
  );
